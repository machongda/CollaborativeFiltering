...
Page({
  ...
  onLoad: function (options) {
    ...
    var lebels = app.globalData.userInfo.lebels.split(",");
    for (var j of lebels) {
      for (var i in this.data.literature)
        if (this.data.literature[i] == j)
          this.data.literatureFlag[i] = true;

      for (var i in this.data.popular)
        if (this.data.popular[i] == j)
          this.data.popularFlag[i] = true;

      for (var i in this.data.culture)
        if (this.data.culture[i] == j)
          this.data.cultureFlag[i] = true;
      for (var i in this.data.life)
        if (this.data.life[i] == j)
          this.data.lifeFlag[i] = true;

      

      for (var i in this.data.manage)
        if (this.data.manage[i] == j)
          this.data.manageFlag[i] = true;

      for (var i in this.data.technology)
        if (this.data.technology[i] == j)
          this.data.technologyFlag[i] = true;
    }
    this.setData({
      literatureFlag: this.data.literatureFlag,
      cultureFlag: this.data.cultureFlag,
      lifeFlag: this.data.lifeFlag,
      manageFlag: this.data.manageFlag,
      popularFlag: this.data.popularFlag,
      technologyFlag: this.data.technologyFlag,
